...
| `scripts/w2_3_unified_table.py` | EXP-014 汇总：per-frame CSV → 统一 7 物体表（historical/EXP-014 分列；中位数口径=EXP-006 metrics.json 约定） | [x] |
...
| `tests/` | se3/metrics/camera/synthetic/config（28）+ 真实 YCB-V（7）+ pose 合成回归（3）+ learn（5）+ render_templates（4）+ geo_init（5）+ foundationpose（11）+ demo（10）+ evaluation_objects registry（9）+ w2_3 统一评测守护（4），共 86 项（本地全量；CI 上数据相关测试自动跳过） | [x] |
